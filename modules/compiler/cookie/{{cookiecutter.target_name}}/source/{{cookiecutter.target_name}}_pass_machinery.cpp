...
}  // namespace
{
    ...
    if (const auto *vecz_vf_flags_env =
      ...
      for (auto r : flags) {
        if (r == "A" || r == "a") {
          ...
          // vectorization options themselves on a per-function basis. Until
          // we've designed a new method, keep the legacy behaviour by
          // re-parsing the "VF" environment variable and look for a "v/V"
          // toggle.
  ...
  PassMachinery::getLateTargetPasses() {
  ...
  VeczPassOpts(llvm::Function & F, llvm::ModuleAnalysisManager &,
    ...
    env_var_opts.vecz_pass_opts->vecz_auto =
  ...
  PassMachinery::addClassToPassNames() {
...
#include "{{cookiecutter.target_name}}_pass_registry.def"
  ...
  PassMachinery::handlePipelineElement(llvm::StringRef Name,
                                       llvm::ModulePassManager & PM) {
  ...
  PassMachinery::registerPassCallbacks() {
...
#include "{{cookiecutter.target_name}}_pass_registry.def"
  ...
  PassMachinery::printPassNames(llvm::raw_ostream & OS) {

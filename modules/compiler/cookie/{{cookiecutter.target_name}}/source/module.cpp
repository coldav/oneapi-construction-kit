  ...
  class CLTargetBuiltinInfo : public compiler::utils::CLBuiltinInfo {
   public:
    ...
    llvm::Value *emitBuiltinInline(
      ...
      if (Builtin) {
        compiler::utils::NameMangler mangler(
            &Builtin->getParent()->getContext());
        llvm::StringRef BaseName = mangler.demangleName(Builtin->getName());
        if (BaseName == "clmul") {
      ...
      return compiler::utils::CLBuiltinInfo::emitBuiltinInline(Builtin, B,
                                                               Args);
    }
  };
}
...
}  // namespace
{
      ...
      {
        {
          ...
          target,
      compiler::BaseContext & context, uint32_t &num_errors, std::string &log)
  ...
  Module::createBinary(cargo::array_view<std::uint8_t> & binary) {
    ...
    return target.withLLVMContextDo([&](llvm::LLVMContext &)
                                        -> compiler::Result {
...

      {
        ...
        bool crashed = !CRC.RunSafely([&] {
          ...
          std::memcpy(object_code.data(), (*linkResult)->getBufferStart(),
                      size);
          linkSuccess = true;
        });
        llvm::CrashRecoveryContext::Disable();
...
#if defined(CA_ENABLE_DEBUG_SUPPORT) || \
    defined(CA_{{cookiecutter.target_name_capitals}} _DEMO_MODE)
  ...
  Module::createKernel(const std::string &) { return nullptr; }
  ...
  Module::getLateTargetPasses(compiler::utils::PassMachinery & pass_mach) {
    ...
    PassMachinery & > (pass_mach).getLateTargetPasses();
  }

  static llvm::TargetMachine *createTargetMachine(const {
    {
      cookiecutter.target_name_capitalize
    }
  } Target & target) {
    ...
    return llvm_target->createTargetMachine(
        ...
        {{cookiecutter.link_shared}} ? llvm::Reloc::Model::PIC_
  ...
  std::unique_ptr<compiler::utils::BILangInfoConcept>
  createSimpleTargetCLBuiltinInfo(llvm::Module * Builtins) {
  ...
  Module::createPassMachinery(llvm::LLVMContext & C) {
    auto *TM = getTargetMachine();
    ...
    auto Callback = [Builtins](const llvm::Module &) {
      ...
      return compiler::utils::BuiltinInfo(
          createSimpleTargetCLBuiltinInfo(Builtins));
      {
        % else - %
      }
      return compiler::utils::BuiltinInfo(
    ...
    PassMachinery > (getTarget(), C, TM, Info, Callback,
  ...
  Module::initializePassMachineryForFrontend(
    ...
    // loop unrolling. - comment from clang source
  ...
  Module::initializePassMachineryForFinalize(compiler::utils::PassMachinery &
                                             pass_mach) const {
